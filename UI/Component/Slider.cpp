...
void Slider::Draw() const {
    // TOODO: [HACKATHON-3-BUG] (4/5): Draw the nested components here, so they are displayed correctly
	Bar.Draw();
    End1.Draw();
    End2.Draw();
	ImageButton::Draw();
...
void Slider::SetValue(float value) {
	if (this->value != value) {
        // TOODO: [HACKATHON-3-BUG] (5/5): Fix the function, so it can set the slider value correctly
        Position.x = (1 - value) * Bar.Position.x + value * (Bar.Position.x + Bar.Size.x);
		// float handlePosition = Bar.Position.x + (value * Bar.Size.x);
        // Position.x = handlePosition;
